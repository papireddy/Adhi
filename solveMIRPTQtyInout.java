...
public class solveMIRPTQtyInout {
	...
	public static void main(String[] args) {
		...
		System.out
				.println("________________Formulation In-Out Qty Done!!!________________");
	}
	...
	private static void processProblem(Instance instance, boolean lp,
		...
		try {
			...
			IloNumVar[][] inQtyVar = createQuantityVariables(cplex, instance,
					"iq");
			IloNumVar[][] outQtyVar = createQuantityVariables(cplex, instance,
					"oq");
	...
	private static void createQuanTranLinkConstraints(IloCplex cplex,
		...
		for (int timeIndex = 0; timeIndex < instance.getTime(); timeIndex++) {
			for (int nodeIndex = 0; nodeIndex < instance.getNoNodes(); nodeIndex++) {
				...
				bw.write(cplex.addEq(inExpr, inQtyVar[timeIndex][nodeIndex])
						+ "\n");
				bw.write(cplex.addEq(outExpr, outQtyVar[timeIndex][nodeIndex])
						+ "\n");
			}
		}
	}
		...
		for (int timeIndex = 0; timeIndex < instance.getTime(); timeIndex++) {
			for (int nodeIndex = 0; nodeIndex < instance.getNoNodes(); nodeIndex++) {
				...
				bw.write(cplex.addEq(
						...
						cplex.sum(invVar[timeIndex][nodeIndex],
								inQtyVar[timeIndex][nodeIndex], cplex.prod(-1,
										outQtyVar[timeIndex][nodeIndex]), cplex
										.constant(-instance.getNode(nodeIndex)
												.getDemand()[timeIndex])))
						+ "");
				bw.newLine();
			}
	...
	private static IloNumVar[][] createQuantityVariables(IloCplex cplex,
		...
		for (int timeIndex = 0; timeIndex < instance.getTime() + 1; timeIndex++) {
			...
			for (int nodeIndex = 0; nodeIndex < instance.getNoNodes(); nodeIndex++) {
				qtyVar[timeIndex][nodeIndex] = cplex.numVar(0,
						Double.MAX_VALUE, IloNumVarType.Float, dir + "^"
								+ timeIndex + "_" + nodeIndex);
	...
	private static void getSolution(IloCplex cplex, Instance instance,
		...
		try {
			...
			for (int timeIndex = 0; timeIndex < instance.getTime(); timeIndex++) {
				solwriter.write("In Quantity Variable for period:" + timeIndex
						+ "\n");
				for (int nodeIndex = 0; nodeIndex < instance.getNoNodes(); nodeIndex++) {
					if (cplex.getValue(inQtyVar[timeIndex][nodeIndex]) != 0) {
						solwriter
								.write(inQtyVar[timeIndex][nodeIndex].getName()
										+ ":"
										+ cplex.getValue(inQtyVar[timeIndex][nodeIndex])
										+ ";\t");
					}
				}
				solwriter.write("Out Quantity Variable for period:" + timeIndex
						+ "\n");
				for (int nodeIndex = 0; nodeIndex < instance.getNoNodes(); nodeIndex++) {
					if (cplex.getValue(outQtyVar[timeIndex][nodeIndex]) != 0) {
						solwriter
								.write(outQtyVar[timeIndex][nodeIndex]
										.getName()
										+ ":"
										+ cplex.getValue(outQtyVar[timeIndex][nodeIndex])
										+ ";\t");
					}
				}
				solwriter.write("************************");
