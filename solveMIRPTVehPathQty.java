...
public class solveMIRPTVehPathQty {
	...
	public static BufferedWriter nodetimelogger = null;

	public static void main(String[] args) {
		String inputfile = "D:\\The Folder\\IRP\\TestCases\\Problems";
		try {
			nodetimelogger = new BufferedWriter(new FileWriter(
					"D:\\The Folder\\IRP\\TestCases\\NodeTimeLog.xls", true));
		} catch (IOException e) {
			e.printStackTrace();
		}
		solveProblems(inputfile);
		try {
			nodetimelogger.close();
		} catch (IOException e) {
			e.printStackTrace();
		}
		System.out
				.println("________________Formulation 5 Done!!!________________");
	}
	...
	private static void processProblem(Instance instance, boolean lp,
		...
		try {
			...
			mapwriter.write("\\documentclass[a4paper]{article}\n"
					+ "\\usepackage{tikz,float,amssymb} \n"
					+ "\\usetikzlibrary{shapes.geometric,positioning} \n"
					+ "\\begin{document} " + "Included Cuts: \n\n");
			...
			if (cplex.solve()) {
				System.out
						.println("Tran Cost:"
								+ cplex.getValue(getTransCost(cplex, instance,
										routeVar)));
				double procTime = ((System.nanoTime() - startTime) / 1E9);
				String cutIndicator = "";
				if (cuts) {
					cutIndicator = "With Cuts";
				} else {
					cutIndicator = "Without Cuts";
				}
				System.out.println(fileName.substring(fileName
						...
						+ procTime
						+ " sec");
				nodetimelogger.write(fileName.substring(fileName
						.lastIndexOf("\\") + 1)
						+ "\t"
						+ cplex.getObjValue()
						+ "\t"
	...
	private static void createConstraints(IloCplex cplex, Instance instance,
		...
		try {
			...
			// // ADD TOGET Valid Solution CUTS
			// addGetValidCuts(cplex, routeVar);
			...
			if (cuts) {
				...
				/*
				 * deleted because z3 take care of this // sigma // z^{tv}_{vj}
				 * + // 1 // * >= sigma sigma y^{tv}_{ij} cuts z2 //
				 * createSumZGYCuts(cplex, instance, routeVar, visitVar);
				 */

				// sigma sigma y^{ti}_{jk} >= sigma z^{ti}_{ij} cuts z3
				createSumYGSumZCuts(cplex, instance, routeVar, visitVar);

				// z_{oj} = 0
				createOriginOfVehicleVisitCuts(cplex, instance, visitVar);

				// z_{ij}+z_{ji}<=sigma y^i_{ik} z4
				createZijZjiCuts(cplex, instance, visitVar, routeVar);

				// y_{ij}+y_{ji}<=sigma y^i_{ik} y5
	...
	private static void addGetValidCuts(IloCplex cplex,
			IloNumVar[][][][] routeVar) throws IloException, IOException {
		bw.write("Add Valid Solution Cuts:");
		bw.newLine();

		bw.write(cplex.addUserCut(cplex.le(
				cplex.sum(routeVar[0][3][2][0], routeVar[0][3][0][2]), 1))
				+ "");
		bw.write(cplex.addUserCut(cplex.le(
				cplex.sum(routeVar[0][6][1][0], routeVar[0][6][0][1]), 1))
				+ "");
		bw.write(cplex.addUserCut(cplex.le(
				cplex.sum(routeVar[0][1][5][6], routeVar[0][1][6][5]), 1))
				+ "");
	}

	private static void createOriginOfVehicleVisitCuts(IloCplex cplex,
			Instance instance, IloNumVar[][][][] visitVar) throws IloException,
			IOException {
		bw.write("z_{j0}=0 cuts:");
		bw.newLine();

		mapwriter.write("$z^{ti}_{ji} = 0$ \n\n");
		...
		for (int timeIndex = 0; timeIndex < instance.getTime(); timeIndex++) {
			for (int vehicleIndex = 0; vehicleIndex < instance.getNoNodes(); vehicleIndex++) {
				for (int nodeIndex = 0; nodeIndex < instance.getNoNodes(); nodeIndex++) {
					bw.write(cplex
							.addEq(visitVar[timeIndex][vehicleIndex][nodeIndex][vehicleIndex],
									0)
							+ "\t");
				}
				bw.newLine();
	...
	private static void createYijYjiCuts(IloCplex cplex, Instance instance,
		...
		mapwriter
				.write("$y^{tv}_{ij} + y^{tv}_{ji} \\leqslant \\sum_{k} y^{ti}_{ik}$ \n\n");
	...
	private static void createZijZjiCuts(IloCplex cplex, Instance instance,
		...
		mapwriter
				.write("$z_{ij}^{tv}+z_{ji}^{tv} \\leqslant \\sum_{k} y^{ti}_{ik}$ \n\n");
	...
	private static void createSumYGSumZCuts(IloCplex cplex, Instance instance,
		...
		mapwriter
				.write("$\\sum_j \\sum_k y^{ti}_{jk} = \\sum_{j} z^{ti}_{ij}$ \n\n");
	...
	private static void createSumZGYCuts(IloCplex cplex, Instance instance,
		...
		mapwriter
				.write("$\\sum z_{ij}^{tv} +1 \\geqslant \\sum y_{ij}^{tv}$ \n\n");
	...
	private static void createVisitRouteCuts(IloCplex cplex, Instance instance,
		...
		bw.newLine();
		mapwriter.write("$z_{ij}^{tv} \\geqslant y_{ij}^{tv}$ \n\n");
		...
		for (int timeIndex = 0; timeIndex < instance.getTime(); timeIndex++) {
			for (int nodeIndex = 1; nodeIndex < instance.getNoNodes(); nodeIndex++) {
				...
				for (int toTimeIndex = timeIndex; toTimeIndex < instance
					...
					if (demandSum >= 0) {
						bw.write(cplex.addUserCut(cplex.ge(
								invVar[timeIndex][nodeIndex],
								cplex.prod(demandSum, expr)))
								+ "");
						bw.newLine();
					} else if (demandSum < 0) {
						bw.write(cplex.addUserCut(cplex.ge(cplex.sum(instance
								.getNode(nodeIndex).getMaxCapacity(), cplex
								.prod(-1, invVar[timeIndex][nodeIndex])), cplex
								.prod(-demandSum, expr)))
								+ "");
						bw.newLine();
					}
	...
	private static void createStartInvDemDiffCuts(IloCplex cplex,
		...
		for (int nodeIndex = 1; nodeIndex < instance.getNoNodes(); nodeIndex++) {
			if (instance.getNode(nodeIndex).getDemand()[0] > instance.getNode(
				...
				bw.write(cplex.addUserCut(cplex.ge(expr, 1)) + "");
				bw.newLine();
			}
		}
	}
	...
	private static void createDemandInvDiffCuts(IloCplex cplex,
		...
		for (int timeIndex = 0; timeIndex < instance.getTime(); timeIndex++) {
			for (int nodeIndex = 1; nodeIndex < instance.getNoNodes(); nodeIndex++) {
				...
				bw.write(cplex.addUserCut(cplex.le(cplex.sum(
						instance.getNode(nodeIndex).getDemand()[timeIndex],
						cplex.prod(-1, invVar[timeIndex][nodeIndex])), allExpr))
						+ "");
				bw.newLine();
			}
		}
	...
	private static void getSolution(IloCplex cplex, Instance instance,
		...
		try {
			...
			mapwriter.write("Objective Value: " + cplex.getObjValue() + "\n\n");
			System.out.println("Number of Nodes:" + cplex.getNnodes()
					+ " 64thing:" + cplex.getNnodes64());
			...
			for (int timeIndex = 0; timeIndex < instance.getTime(); timeIndex++) {
				...
				for (int vehicleIndex = 0; vehicleIndex < instance.getNoNodes(); vehicleIndex++) {
					mapwriter.write("Quantity Transfer Variables($x_{ij}^{"
							+ timeIndex + "" + vehicleIndex + "}$),\n\n");
					mapwriter.write("Visit Variables($z_{ij}^{" + timeIndex
							+ "" + vehicleIndex
							+ "}$) and Route Variables($y_{ij}^{" + timeIndex
							+ "" + vehicleIndex + "}$)for time period:"
							+ timeIndex + ", vehicle:" + vehicleIndex + "\n\n");
					boolean open = true;
					for (int fromNodeIndex = 0; fromNodeIndex < instance
							.getNoNodes(); fromNodeIndex++) {
						for (int toNodeIndex = 0; toNodeIndex < instance
								...
								mapwriter
										.write(getArcforMap(
												...
												getformattedNumber(cplex
														.getValue(qtyVar[timeIndex][vehicleIndex][fromNodeIndex][toNodeIndex]))));
							}
						}
						solwriter.write("\t");
					}
					...
					solwriter.write("Visit and Route Variables for time:"
							+ timeIndex + "\n");
			...
			solwriter.write("Inventory at the end of the horizon:\n");
			for (int nodeIndex = 0; nodeIndex < instance.getNoNodes(); nodeIndex++) {
				solwriter
